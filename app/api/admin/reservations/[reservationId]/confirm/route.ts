...
// POST /api/admin/reservations/[reservationId]/confirm — admin konfirmasi bayar reservasi
export async function POST(req: NextRequest, { params }: { params: Promise<{ reservationId: string }> }) {
  try {
    ...
    return successResponse(updated, `Pembayaran reservasi slot ${reservation.slot.slotCode} berhasil dikonfirmasi`);
  } catch (err) {
